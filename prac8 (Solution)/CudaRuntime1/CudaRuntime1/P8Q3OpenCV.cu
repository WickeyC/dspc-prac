...
int main(int argc, char **argv)
{
     ...
     unsigned char *d_rgb_image = NULL; //array for storing rgb data on device
     unsigned char *h_grey_image = NULL;
     unsigned char *d_grey_image = NULL; //host and device's grey data array pointers
     ...
     cudaMalloc((void**)&d_rgb_image, sizeof(unsigned char*) * total_pixels);
     cudaMalloc((void**)&d_grey_image, sizeof(unsigned char*) * total_pixels);

     cudaMemset(d_grey_image, 0, sizeof(unsigned char) * total_pixels);

     //@@copy host rgb data array to device rgb data array
     cudaMemcpy(d_rgb_image, h_rgb_image, sizeof(unsigned char*) * total_pixels, cudaMemcpyHostToDevice);
...
size_t loadImageFile(unsigned char *grey_image, const std::string &input_file, int *rows, int *cols)
{
     ...
     //img_data = cv::imread(input_file, IMREAD_COLOR);
     if (img_data.empty())
     {
